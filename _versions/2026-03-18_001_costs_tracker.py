# ==============================================================================
# COSTS TRACKER — Crisalia LLM Cost Monitor v2.1
...
async def log_cost(model: str, endpoint: str, tokens_input: int, tokens_output: int) -> float:
    """
    Registra el costo de una llamada LLM en Supabase de forma asíncrona.
...
async def get_summary() -> dict:
    ...
    y agrega en Python. Esto no escala a miles de usuarios.
    Migrar a una de estas dos opciones en Supabase:
      A) VIEW de PostgreSQL con GROUP BY por modelo, endpoint y día
         → SELECT * from v_costs_summary (una sola fila por grupo, sin descarga masiva)
      B) Función RPC en Supabase: supabase.rpc('get_costs_summary')
         → ejecuta el SQL en la base y devuelve solo el JSON final
    ─────────────────────────────────────────────────────────────────────────
    """
    ...
    by_model    = {}
    by_endpoint = {}
    daily_dict  = {}   # (date, model) -> cost acumulado
